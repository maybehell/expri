 ...
 * 函数名称：main
 *
 * 功能描述：自动识别传感器类型（温湿度传感器或光敏传感器），并显示对应数据
 *
 * 参    数：无
 *
 * 返 回 值：无
...
/* 检测SHT10是否存在，带超时机制 */
unsigned char detectSHT10(void)
{
  unsigned char i;
  ...
  /* 检查ACK位 */
  SDA = 1;
  IO_DIR_PORT_PIN(1, 1, IO_IN);  // 设置为输入，读取ACK
  QWait(); QWait();
  SCL = 1;
  QWait(); QWait();
  
  if(SDA == 1)  // 没有收到ACK，传感器不存在
  {
    error = 1;
  }
  ...
  for(timeout = 0; timeout < 500000; timeout++)
  {
    if(SDA == 0)
    {
      return 1;  // SHT10存在且响应正常
    }
  }
  
  return 0;  // 超时，SHT10不存在
}
...
void main()
{
  ...
  UINT8 adc0_value[2];
  float num = 0;
  unsigned char sensorType = 0;  // 0: 未知, 1: 温湿度传感器, 2: 光敏传感器
  ...
  GUI_PutString5_7(10,22,"Detecting...");                  // 显示检测中
  LCM_Refresh();
  ...
  {
    GUI_PutString5_7(10,22,"Light:");
  }
  LCM_Refresh();
  
  while(1)
  {
    if(sensorType == 1)  // 温湿度传感器
    {
      th_read(&tempera, &humidity);
      sprintf(s, (char*)"%d%d C  ", ((INT16)((int)tempera / 10)), ((INT16)((int)tempera % 10)));
      GUI_PutString5_7(48,22,(char *)s);
      LCM_Refresh();
      sprintf(s, (char*)"%d%d %%  ", ((INT16)((int)humidity / 10)), ((INT16)((int)humidity % 10)));
      GUI_PutString5_7(48,35,(char *)s);
      LCM_Refresh();
    }
    else  // 光敏传感器
    {
      ...
      num = (adc0_value[1] * 256 + adc0_value[0]) * 3.3 / 8192;
      num /= 4;
      num = num * 913;
      sprintf(s, (char*)"%d%d%d%d lx  ", ((INT16)((int)num/1000)), ((INT16)((int)num%1000/100)), ((INT16)((int)num%100/10)), ((INT16)((int)num%10)));
      GUI_PutString5_7(48,22,(char *)s);
      LCM_Refresh();
    }
  }
