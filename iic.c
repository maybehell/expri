...
void ctrPCA9554FLASHLED(uint8 led)   
{
  ...
  if(ISendStr(0x40,0x03,&output,1))    //写配置
  {
    ...
    {
     case 1:     
     output = PCA9554ledstate & 0x01;  //判断所控制位的当前状态 
     ...
     if(ISendStr(0x40,0x01,&output,1))    //写输出寄存器
    {
      ...
      {
        PCA9554ledstate = *data;          //保存输出寄存器当前状态。
      }
    }
  }
}
 ...
 * 函数名称：ctrPCA9554Key
 *
 * 功能描述：读取相应IO扩展芯片PCA9554的按键值
 *                    
 * 参    数：无          
 *
 * 返 回 值：无
 *  
 * 注    意：PCA9554的地址为：0x42
 *****************************************************************************/ 
uint8 ctrPCA9554Key()
{
  uint8 input = 0xff;
  uint8 *data = 0;
  if(ISendStr(0x42,0x03,&input,1))  //配置PCA9554寄存器
  {   
    if(ISendByte(0x42,0x00))        //写命令
    {
     if(IRcvByte(0x42,data))        //读输入寄存器
     {
       return *data;
      }
    }
 ...
 * 函数名称：GetKeyInput
 *
 * 功能描述：调用按键输入读取函数的到按键值，然后对输入值进行编码
 *                    
 * 参    数：无          
 *
 * 返 回 值：ksave0 - 按键编码值
...
uint8 GetKeyInput()
{
  uint8 key;
  uint8 ksave0 = 0;
    key = ctrPCA9554Key();     //读取输入值
    if (key == 0xfe)           //对输入值进行编码
    {
      ksave0 = 1;
    }
    else if (key == 0xfd)
